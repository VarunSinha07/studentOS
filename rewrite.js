...
const MOCK_PLAN = [
  { day: 1, title: "Review fundamentals & syllabus", description: "Read through core concepts and organize materials." },
  { day: 2, title: "Deep dive: Core Concepts", description: "Focus on chapters 1-3. Do practice exercises." },
  { day: 3, title: "Practice & Application", description: "Solve past papers and identify weak areas." },
  { day: 4, title: "Weak area revision", description: "Re-read chapters where you struggled yesterday." },
  { day: 5, title: "Mock Test 1", description: "Take a full timed mock exam under test conditions." },
  { day: 6, title: "Final Review", description: "Review all formulas, definitions, and flashcards." },
  { day: 7, title: "Rest & Mental Prep", description: "Light review. Get adequate sleep for tomorrow." },
];
...
export function StudyPlannerApp({
  ...
  useEffect(() => {
    if (session?.user?.id) {
      getSubjects(session.user.id).then((data) => {
        setSubjects(data);
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedSubject) {
      setError("Please select a subject or create one in the Knowledge Base.");
      return;
    }
    if (!examDate) {
  ...
  return (
    <motion.div
      ...
      dragConstraints={{
        top: 0,
        left: 0,
        right: Math.max(0, typeof window !== 'undefined' ? window.innerWidth - 700 : 0),
        bottom: Math.max(0, typeof window !== 'undefined' ? window.innerHeight - 550 : 0),
      }}
    ...
    >
      ...
      <div className="title-bar h-12 bg-[#1a1a24]/60 flex items-center px-4 justify-between border-b border-white/5 backdrop-blur-xl cursor-grab active:cursor-grabbing shrink-0 transition-colors">
        <MacDots onClose={onClose} onMinimize={handleMinimize} onMaximize={handleMaximize} />
        ...
        <div className="w-[50px]"><span className="opacity-0">spacer</span></div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden relative scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10 p-6">
        ...
        <div className="max-w-xl mx-auto relative z-10">
          <div className="mb-8">
            <h1 className="text-3xl font-extrabold text-white tracking-tight mb-1 font-serif flex items-center gap-3">
          ...
          <form onSubmit={handleGenerate} className="bg-[#1c1c24]/80 border border-white/10 rounded-xl p-5 shadow-xl backdrop-blur-md mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              <div className="space-y-1.5">
                <label className="text-xs font-bold tracking-widest text-slate-500 uppercase">Subject</label>
                {loading ? (
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold tracking-widest text-slate-500 uppercase">Exam Date</label>
                <div className="relative">
                  <input
                    type="date"
                    value={examDate}
            ...
            {plan && !generating && (
              ...
              >
                <div className="mb-6 flex items-center justify-between">
                   <h2 className="text-lg font-bold text-white tracking-tight">Your Preparation Timeline</h2>
                   <div className="text-xs px-2 py-1 bg-orange-500/10 text-orange-400 rounded-md border border-orange-500/20">Total: {plan.length} Days</div>
                </div>
                  ...
                  {plan.map((item, i) => (
                    ...
                    >
                      ...
                      <div className="shrink-0 w-12 h-12 bg-[#1c1c24] border border-orange-500/30 rounded-xl flex flex-col items-center justify-center shadow-[0_0_15px_rgba(249,115,22,0.1)] relative z-10">
                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest leading-none">Day</span>
                        <span className="text-orange-400 font-black leading-none mt-0.5">{item.day}</span>
                      </div>
...
fs.writeFileSync('e:/placement-training/studentos/components/os-windows/StudyPlannerApp.tsx', code);
